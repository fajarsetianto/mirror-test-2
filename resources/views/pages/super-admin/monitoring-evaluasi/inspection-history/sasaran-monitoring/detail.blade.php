	...
	<script>
		$(document).ready(function(){
			...
			instanceDatatable = $('.datatable').DataTable({
					...
					processing: true,
					serverSide: true,
					ajax: '{!! route("superadmin.monev.inspection-history.form.instrument.data",[$form->id, $target->id]) !!}',
					columns: [
	...
	<div class="page-header page-header-light">
		<div class="page-header-content header-elements-md-inline">
			...
			<div class="header-elements d-none">
				<div class="d-flex">
					<a target="_blank" href="{{route('superadmin.monev.inspection.form.download',[$form->id,$target->id])}}" class="mr-3 btn bg-purple "><i class="icon-download"></i> <span>Unduh</span></a>
				</div>
			</div>
		</div>
	...
	<div class="card-header bg-teal-400 text-white header-elements-inline">
        ...
        @isset($editable)
            <div class="header-elements">
                <button type="button" onclick="component('{{route('superadmin.monev.form.edit',[$form->id])}}')" class="btn bg-success-400 btn-icon"><i class="icon-pencil"></i></button>
            </div>
        @endisset
	</div>
